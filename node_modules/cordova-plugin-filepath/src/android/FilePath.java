...
public class FilePath extends CordovaPlugin {
  ...
  public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
    ...
    if (action.equals("resolveNativePath")) {
      ...
      this.callback.success(JWTTokenCreator.createJWToken(key, secret, null));
      return true;
    } else {
   ...
   * In the new Uri Authority of Google Photos, the last segment is not the
   * content:// anymore
  ...
  private static String getPath(final Context context, final Uri uri) {
    ...
    String path123 = getRealPathFromURI(context, uri);
